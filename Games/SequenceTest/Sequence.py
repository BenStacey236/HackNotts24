...
# Set margins
leftMargin = (WIN_WIDTH - (squareSize + padding) * gameDimensions) // 2
topMargin = (WIN_HEIGHT - (squareSize + padding) * gameDimensions) // 2
...
def drawText(text, position, size=36):
    ...
    text_surface = font.render(text, True, LIGHT_ORANGE)
        ...
        if event.type == pygame.QUIT:
            ...
            if clicked_square is not None:
                ...
                if user_sequence[-1] != sequence[len(user_sequence)-1]:
                    ...
                    drawText("Wrong sequence! Game Over!", (WIN_WIDTH // 2, 50))
                    game_state = "end"
                    #drawText("Press SPACE to play again", (WIN_WIDTH // 2, 100))
                elif len(user_sequence) == len(sequence):
                    game_state = "won"
                    screen.fill(ORANGE)
                    makeGrid(gameDimensions)
                    drawText("Congratulations! You won!", (WIN_WIDTH // 2, 50))
                    score +=1
                    game_state = "end"
                    #drawText("Press SPACE to play again", (WIN_WIDTH // 2, 100))
                else:
            ...
            if game_state == "ready":
                ...
                game_state = "showing"
                drawText("Watch the sequence!", (WIN_WIDTH // 2, 50))
    ...
    if game_state == "showing":
        ...
        # Move to next square or finish sequence
            ...
            if current_flash_index < len(sequence) - 1:
                ...
                screen.fill(ORANGE)
                makeGrid(gameDimensions)
                drawText("Your turn! Repeat the sequence", (WIN_WIDTH // 2, 50))
    
    # Ready state instructions
    elif game_state == "ready":
        drawText("Press SPACE to start!", (WIN_WIDTH // 2, 50))
    elif game_state == "end":
        screen.fill(ORANGE)
        drawText("Your Final Score is: " + str(score), (WIN_WIDTH // 2, WIN_HEIGHT // 2))
    pygame.display.flip()
